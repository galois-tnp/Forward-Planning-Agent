...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        return any([~i in actionA.effects for i in actionB.effects] + [~i in actionB.effects for i in actionA.effects])
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        return any([~j in actionA.preconditions for j in actionB.effects]) or any([~j in actionB.preconditions for j in actionA.effects])
        raise NotImplementedError
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        return any(self.parent_layer.is_mutex(itm1,itm2) for itm1 in actionA.preconditions for itm2 in actionB.preconditions) \
                or any(self.parent_layer.is_mutex(itm2, itm1) for itm1 in actionA.preconditions for itm2 in actionB.preconditions)
...
class PlanningGraph:
    ...
    def h_setlevel(self):
        ...
        def NoMutex(layer):
            for x1,x2 in combinations(self.goal,2):
                if layer.is_mutex(x1,x2):
                    return False
            return True
        level = 0
        while not self._is_leveled:
            layer = self.literal_layers[-1]
            if GoalSeen(layer) and NoMutex(layer):
                return level
